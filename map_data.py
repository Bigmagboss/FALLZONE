...
def list_saved_maps():
    ...
    for path in MAPS_DIR.glob('*.json'):
        try:
            ...
            saved_maps.append({
                'map_id': path.stem,
                'map_name': _clean_map_name(raw.get('map_name', path.stem.replace('_', ' ').title())),
            })
...
def is_default_map(map_id):
    if map_id is None:
        return False
    return get_default_map_id() == _safe_map_id(map_id)
...
def save_map_data(data, map_id=None):
    ...
    desired_id = _safe_map_id(map_name)
    target_id = _unique_map_id(desired_id, ignore_id=old_id)
